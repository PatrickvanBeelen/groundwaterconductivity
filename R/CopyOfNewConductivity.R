...
MaakKolomMeth<-function(metveldgemiddelden=LMMstyleBroadInputfile,celcius=celcius,add_bicarbonate=add_bicarbonate,add_phosphate=add_phosphate){
...
Conductivity<-function(inputfilename="data/metingen.rda",inputstyle='KRWQCinput',outputstyle='KRWQCoutput',celcius=25){
  ...
  if(inputstyle=='Stuyfzand'){
    ...
    inputfile=ReadListPointComma(inputfilename)
    myrownames=row.names(inputfile)
    s=cbind(inputfile,myrownames)
    matrixnamen=c('xal',"xca","xcl","xfe","xhv","xk","xmg","xmn","xna","xnh4","xno3",'xpo4',"xso4",'xecv','xzn','xhco3','xco3','myrownames')
    l=data.frame(matrix(nrow=length(s[,1]),ncol=length(matrixnamen),0)) 
    names(l)=matrixnamen
    l$xcl=s$cl
    ...
    save(LMMstyleBroadInputfile,file = "originalinputfile.rda")
  }  
  z=MaakKolomMeth(metveldgemiddelden=LMMstyleBroadInputfile,celcius=celcius,add_bicarbonate = add_bicarbonate,add_phosphate = add_phosphate)
  z=Blanquet(z)
  z=Logan(z)
  z=Dunlap(z)
  z=McNeal(z)
  ...
  if(outputstyle=='BroadLMMstyle'){
    ...
    save(metallegeleidbaarheid,file='metallegeleidbaarheid.rda')
    mycols=c(names(LMMstyleBroadInputfile),'cl','so4','no3','na','k','ca','mg','po4','hco3','xhco3e','percentageverschil_xecv_ec25','ec25','prinslabel','ec25_xecv_sr')
    metgeleidbaarheid=h[,mycols]
    # WriteListPointComma(metgeleidbaarheid,filename='metgeleidbaarheid.csv')
    save(metgeleidbaarheid,file='metgeleidbaarheid.rda')
  }
...
Stuyfzandtest=Conductivity(inputfilename="data/StuyfzandTable31.csv",inputstyle='Stuyfzand',outputstyle='Stuyfzandstyle',celcius=25)
